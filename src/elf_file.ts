...
export class ELF_File {
    ...
    constructor(filePath: string) {
        ...
        if (!Utils.isDefined(progHeadersBuffer) || progHeadersBuffer.isNull()) {
            dwarfFS.fclose(_file);
            throw new Error("Failed to allocate Memory!");
        }

        if (dwarfFS.fseek(_file, this.fileHeader.e_phoff, FileSystem.SeekDirection.SEEK_SET) !== 0) {
            dwarfFS.fclose(_file);
            throw new Error("Failed to read from File!");
        }
        ...
        if (!Utils.isDefined(strTableBuffer) || strTableBuffer.isNull()) {
            dwarfFS.fclose(_file);
            throw new Error("Failed to allocate Memory!");
        }

        //get strtable
        if (dwarfFS.fseek(_file, this.fileHeader.e_shoff + this.fileHeader.e_shentsize * this.fileHeader.e_shstrndx, FileSystem.SeekDirection.SEEK_SET) !== 0) {
            dwarfFS.fclose(_file);
            throw new Error("Failed to read from File!");
        }
        ...
        if (dwarfFS.fseek(_file, section.sh_offset, FileSystem.SeekDirection.SEEK_SET) !== 0) {
            dwarfFS.fclose(_file);
            throw new Error("Failed to read from File!");
        }

        let strSectionBuffer = dwarfFS.allocateRw(section.sh_size);
        if (!Utils.isDefined(strSectionBuffer) || strSectionBuffer.isNull()) {
            dwarfFS.fclose(_file);
            throw new Error("Failed to allocate Memory!");
        }
        ...
        while (pos < section.sh_size) {
            ...
            if (Utils.isDefined(str) && str !== null && str.length > 0) {
        ...
        if (!Utils.isDefined(sectionsBuffer) || sectionsBuffer.isNull()) {
            dwarfFS.fclose(_file);
            throw new Error("Failed to allocate Memory!");
        }

        if (dwarfFS.fseek(_file, this.fileHeader.e_shoff, FileSystem.SeekDirection.SEEK_SET) !== 0) {
            dwarfFS.fclose(_file);
            throw new Error("Failed to read from File!");
        }
        ...
        for (let i = 0; i < this.fileHeader.e_shnum; i++) {
            ...
            if (section.name === ".init_array") {
                ...
                if (!Utils.isDefined(initArrayBuffer) || initArrayBuffer.isNull()) {
                    dwarfFS.fclose(_file);
                    throw new Error("Failed to allocate Memory!");
                }
                if (dwarfFS.fseek(_file, section.sh_offset, FileSystem.SeekDirection.SEEK_SET) !== 0) {
...
export namespace ELF_File {
    export class ELF_Header {
        ...
        constructor(dataPtr: NativePointer) {
            //parse header
            if (Utils.isDefined(dataPtr) && !dataPtr.isNull()) {
                this.e_ident = [];
